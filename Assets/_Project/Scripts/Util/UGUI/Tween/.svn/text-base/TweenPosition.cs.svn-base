	...
	void Update () {
		if (canTween) {
			...
			if (curTime < delay) {
				...
				if (temp >= during) {
					transform.localPosition = Vector3.LerpUnclamped(from, to, curve.Evaluate(1));
                    if (callback != null) {
						callback ();
					}
					enabled = false;
					...
					transform.localPosition = Vector3.LerpUnclamped (from, to, curve.Evaluate (temp / during));
				}
			}
		}
	}
